...
--  https://supabase.com/dashboard/project/dwutbaformujqrflxvkp/sql
--  Then click RUN
-- ================================================================
...
-- Step 2: Tables (safe - won't overwrite if they exist)
...
-- Step 4: Drop old policies and recreate clean
...
-- Step 5: Auto-create profile trigger (fires on every signup)
...
insert into public.menu_items (name, description, price, category, image_url, offer_percent) values
  ...
  ('Penne Arrabbiata',    'San Marzano tomato, garlic, bird eye chili, basil',     10.50,'Pasta',   '🍝',20),
...
select 'profiles table' as check, count(*)::text as rows from public.profiles
union all
select 'menu_items table', count(*)::text from public.menu_items
union all
select 'trigger exists', count(*)::text from information_schema.triggers where trigger_name = 'on_auth_user_created'
union all
select 'RLS on profiles', rowsecurity::text from pg_tables where tablename='profiles' and schemaname='public';
